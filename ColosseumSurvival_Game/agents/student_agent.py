...
class StudentAgent(Agent):
    """
    Iterative Deepening Heuristic Alpha Beta Tree Search Agent
    Optimized with a history table and move ordering 
    """
    def __init__(self):
        ...
        self.history_table = {}   # history table to cache moves to its evaluation value
        self.time_limit = 1.99   # create some buffer time to 2s 
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        ...
        # can give more time for first step? Not really necessary
        ...
        # find a cheap panic move in case we lack time
        r, c, dir = self.get_random_walk_move(chess_board, my_pos, adv_pos, max_step)
        self.curr_best = tuple(((r,c), dir))
        ...
        try:
            ...
            # clear the cache as states will never appear again
            self.history_table.clear()
    ...
    def alphabeta (self, chess_board, my_pos, adv_pos, max_step, max_depth):
        ...
        A history table is used to optimize the search time.
        ...
        def terminal_test(chess_board, my_pos, adv_pos, max_step, curr_depth):
            ...
            node of a game tree). A history table is used as a cache to save evaluation 
            time for seen game states and to order moves on the next iteration.
            ...
            if (chess_board.data.tobytes(), my_pos, adv_pos) in self.history_table:
                # cache hit in history table
                eval = self.history_table.get((chess_board.tostring(), my_pos, adv_pos))
            ...
            if isEnd:
                if score > 0:
                    # winning so utility is pushed to the pos extreme
                    eval = score * 1000
                    self.history_table[(chess_board.data.tobytes(), my_pos, adv_pos)] = eval
                    return eval
                elif score < 0:
                    # losing so utility is pushed to the neg extreme
                    eval = score * 1000
                    self.history_table[(chess_board.data.tobytes(), my_pos, adv_pos)] = eval
                    return eval
                else:
        ...
        def max (chess_board, my_pos, adv_pos, max_step, alpha, beta, curr_depth):
            ...
            if eval is not None:
                ...
                for move in valid_moves:
                    ...
                    if (eval_board.data.tobytes(), (r, c), adv_pos) in self.history_table:
                        # game state in cache
                        move_to_shallow_eval[move] = self.history_table[(eval_board.data.tobytes(), (r, c), adv_pos)]

                # order the valid moves based on decreasing order of its evaluation if they are in the cache
        ...
        def min (chess_board, my_pos, adv_pos, max_step, alpha, beta, curr_depth):
            ...
            if eval is not None:
                ...
                for move in valid_moves:
                    ...
                    if (eval_board.data.tobytes(), adv_pos, (r, c)) in self.history_table:
                        # game state in cache
                        move_to_shallow_eval[move] = self.history_table[(eval_board.data.tobytes(), adv_pos, (r, c))]
                        
                # order the valid moves based on increasing order of its evaluation if they are in the cache
    ...
    def get_number_of_reachable_tiles(self, chess_board, my_pos, adv_pos, max_step):
        """Calculate and return the number of reachable tiles from the input game state
        ...
        --------
        num: int
            the number of reachable tiles from my_pos
        """
        # BFS
        all_queue = []
        state_queue = [(my_pos, 0)]
    ...
    def get_random_walk_move(self, chess_board, my_pos, adv_pos, max_step):
        """return a random walk move, modified from random agent
        ...
        --------
        r, c, dir: int, int, int
            parameters that defined a move (a random walk move)
        """
